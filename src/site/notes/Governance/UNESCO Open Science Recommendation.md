---
{"dg-publish":true,"permalink":"/governance/unesco-open-science-recommendation/","dg-note-properties":{"name":"UNESCO Recommendation on Open Science","aliases":["UNESCO Open Science Recommendation"],"website":"https://www.unesco.org/en/open-science","type":"policy","scope":"international","domain":["multimodal"],"status":"active","founded":2021,"parent_org":"UNESCO"}}
---


# UNESCO Recommendation on Open Science (2021)

## Overview
The UNESCO Recommendation on Open Science was adopted by UNESCO Member States in November 2021 at the 41st General Conference. It is the **first international normative instrument on open science**, providing a common definition, shared values, guiding principles, and areas of action for open science policy globally. As a UNESCO Recommendation (rather than a Convention), it is not legally binding but represents strong political commitment from 193 member states and provides a framework for national open science policy development.

The Recommendation directly influenced and aligns with [[Governance/Ouvrir la Science\|Ouvrir la Science]] (France), [[Governance/EC Open Science Policy\|EC Open Science Policy]] (EU), and other national open science plans.

## Four Pillars
The Recommendation defines open science across four interconnected pillars:

### 1. Open Scientific Knowledge
- Open access to scientific publications (preprints, OA journals, repositories)
- Open research data (FAIR principles, data sharing)
- Open educational resources
- Open source software and source code
- Open hardware for scientific instruments

### 2. Open Science Infrastructures
- Non-commercial, publicly owned infrastructure
- Interoperability and data standards
- Long-term sustainability of open repositories
- Multilingual and multicultural accessibility

### 3. Open Engagement with Society
- Citizen science and participatory research
- Science communication and public engagement
- Open science education for researchers and the public

### 4. Open Dialogue with Other Knowledge Systems
- Recognition of indigenous and traditional knowledge
- Inclusive open science that does not impose Western frameworks
- Epistemic diversity in scientific practices

## Key Definitions
The Recommendation provides the internationally agreed definition of open science:

> *"Open science is defined as an inclusive construct that combines various movements
> and practices aiming to make multilingual scientific knowledge openly available,
> accessible and reusable for everyone, to increase scientific collaborations and
> sharing of information for the benefits of science and society, and to open the
> processes of scientific knowledge creation, evaluation and communication to
> societal actors beyond the traditional scientific community."*

## Implementation
- UNESCO monitors implementation through biennial reports from member states
- **Global Open Science Conference** — UNESCO convenes regular ministerial-level meetings on open science implementation
- France reports implementation via [[Governance/CoSO\|CoSO]] and the [[Governance/Barometre Science Ouverte\|Barometre Science Ouverte]]
- The Recommendation explicitly endorses [[Governance/FAIR Principles\|FAIR Principles]] as the technical framework for open research data

## Connections
- Aligns with: [[Governance/FAIR Principles\|FAIR Principles]], [[Governance/Ouvrir la Science\|Ouvrir la Science]], [[Governance/EC Open Science Policy\|EC Open Science Policy]]
- French implementation: [[Governance/CoSO\|CoSO]], [[Governance/Ouvrir la Science\|Ouvrir la Science]], [[Governance/Barometre Science Ouverte\|Barometre Science Ouverte]]
- Partners: [[Governance/GO FAIR\|GO FAIR]], [[Governance/EOSC\|EOSC]], [[Governance/OpenAIRE\|OpenAIRE]], [[Governance/RDA\|RDA]]
- Relevant to: [[Actors/Paris Brain Institute\|Paris Brain Institute]] (provides normative framework for ICM open science commitments; referenced in CNRS/Inserm open science policies)

## Resources
- https://www.unesco.org/en/open-science
- https://doi.org/10.54677/MNMH8546 (full text of the Recommendation)
- https://www.unesco.org/en/open-science/about (UNESCO open science programme)

