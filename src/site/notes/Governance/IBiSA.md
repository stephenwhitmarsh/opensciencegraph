---
{"dg-publish":true,"permalink":"/governance/i-bi-sa/","dg-note-properties":{"name":"Infrastructures en Biologie Santé et Agronomie","aliases":["IBiSA","GIS IBiSA"],"website":"https://www.ibisa.net","type":"infrastructure","scope":"french","domain":["multimodal"],"status":"active","founded":2007,"parent_org":"CNRS / INSERM / INRAE / CEA / INRIA / INCa / France Universités / MESR"}}
---


# IBiSA — Infrastructures en Biologie Santé et Agronomie

## Overview
IBiSA is a **Groupement d'Intérêt Scientifique (GIS)** founded in 2007 under the impulsion of the MESR, the major French research organisations, and France Universités. Its core mission is to identify, label, and fund **technological platforms and biological resource centres (CRB)** of regional or national significance across all domains of biology, health, and agronomy. IBiSA is the unique instrument of shared financing common to all its partner institutions in life sciences.

IBiSA played a foundational role in shaping the current French research infrastructure landscape: its labelling and structuring actions between 2007 and 2013 directly enabled the emergence of the first **Infrastructures Nationales en Biologie-Santé (INBS)**, and the vast majority of platforms now listed on the MESR national roadmap carry the IBiSA label.

For [[Actors/Paris Brain Institute\|Paris Brain Institute]], IBiSA is directly relevant as the labelling body for two of its core technological platforms: [[Actors/CENIR\|CENIR]] and [[Actors/iGENSEQ\|iGENSEQ]].

## Structure and Governance
IBiSA operates as a GIS — a contractual consortium with no legal personality of its own, governed by its eight member institutions: CNRS, INSERM, INRAE, CEA, INRIA, INCa (Institut National du Cancer), France Universités, and the MESR (via DGRI and DGES). The GIS is administered by a secretariat based in Paris and coordinated by a steering committee drawn from its member organisations.

## Missions
IBiSA carries out four main types of activity:

**Labelling** — IBiSA runs annual competitive calls (appels d'offres Plateformes and CRB) to award its label to platforms and biological resource centres demonstrating scientific excellence, openness to the community, and quality management. The IBiSA label is a nationally recognised mark of quality for French life sciences platforms and is a prerequisite for membership in most INBS.

**Funding** — IBiSA provides direct financial support to labelled platforms for equipment investment and methodological or technological development projects. It is the only funding instrument shared across all member institutions in the life sciences.

**Quality support** — IBiSA promotes quality management and certification among its labelled platforms, including ISO-aligned approaches, and organises training workshops on quality practice and ecological footprint.

**INBS coordination** — IBiSA acts as intermediary and facilitator between the full network of 27 INBS and their supervising institutions. It participates in the INBS club (an informal coordination forum), advises on structural and operational optimisation, and supports the creation of regional governance structures and disciplinary networks.

## Relationship to INBS
All platforms within the [[Governance/INBS\|INBS]] ecosystem are typically IBiSA-labelled. IBiSA thus operates one level below the INBS tier: where INBS are the federated national infrastructures (e.g. [[Governance/France BioImaging\|France BioImaging]], [[Governance/France Life Imaging\|France Life Imaging]], [[Governance/IFB\|IFB]]), IBiSA is the body that individually labels and funds the constituent platforms that make up those infrastructures. This layered relationship means IBiSA's label is effectively the entry point into the national infrastructure system.

## [[Actors/Paris Brain Institute\|ICM]]-Labelled Platforms
Two [[Actors/Paris Brain Institute\|Paris Brain Institute]] core platforms hold the IBiSA label:
- **[[Actors/CENIR\|CENIR]]** — multimodal human neuroimaging (MRI, PET-MRI, MEG, EEG, TMS); IBiSA-labelled platform within [[Governance/France Life Imaging\|France Life Imaging]]
- **[[Actors/iGENSEQ\|iGENSEQ]]** — next-generation sequencing; IBiSA-labelled platform for genomics

## Connections
- Type: GIS (Groupement d'Intérêt Scientifique)
- Member institutions: CNRS, INSERM, INRAE, CEA, INRIA, INCa, France Universités, MESR
- Umbrella: [[Governance/INBS\|INBS]] (coordinates between IBiSA-labelled platforms and the INBS tier)
- Labelled [[Actors/Paris Brain Institute\|ICM]] platforms: [[Actors/CENIR\|CENIR]], [[Actors/iGENSEQ\|iGENSEQ]]
- Related infrastructures: [[Governance/France BioImaging\|France BioImaging]], [[Governance/France Life Imaging\|France Life Imaging]], [[Governance/IFB\|IFB]]
- Policy alignment: [[Governance/ANR Open Science Policy\|ANR Open Science Policy]], [[Governance/Ouvrir la Science\|Ouvrir la Science]], [[Governance/CNRS Open Science\|CNRS Open Science]]

## Resources
- https://www.ibisa.net
- https://www.ibisa.net/decouvrir-ibisa.html (missions and history)
- https://www.ibisa.net/actualites/ibisa-infrastructures-nationales-en-biologie-sante-33.html (IBiSA and INBS)
- https://www.ibisa.net/plateformes/centre-neuroimagerie-recherche-cenir-480.html (CENIR entry)
- https://www.ibisa.net/plateformes/igenseq-663.html (iGenSeq entry)
